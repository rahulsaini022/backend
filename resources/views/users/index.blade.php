...
@section('content')
    ...
    <table class="table table-bordered">
        ...
        @foreach ($data as $key => $user)
            <tr>
                ...
                @can('change-status')
                    <td class="slideBtnOuter"><span class="d-none">{{ $user->status }}</span>
                ...
                <td>
                    ...
                    @can('user-delete')
                        {!! Form::open([
                            'method' => 'DELETE',
                            'route' => ['users.destroy', Auth::user()->roles[0]->name, $user->id],
                            'style' => 'display:inline',
                        ]) !!}
                        {!! Form::submit('Delete', [
                            'onclick' => 'return confirm("You want to delete user")',
    ...
    <div class="modal fade" id="addUserModal" tabindex="-1" role="dialog" aria-labelledby="addUserModalLebal"
        aria-hidden="true">
    ...
    <div class="modal fade" id="editUserModal" tabindex="-1" role="dialog" aria-labelledby="editUserModalLebal"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-green">
    ...
    <div class="modal fade" id="showUserModal" tabindex="-1" role="dialog" aria-labelledby="showUserModalLebal"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-green">
    ...
    <script>
        $(function() {
            ...
            $('.cm-toggle').change(function() {
                ...
                $.ajax({
                    type: "Post",
                    dataType: "json",
                    url: "{{ route('status', Auth::user()->roles[0]->name) }}",
                    data: {
                        'status': status,
                        'id': user_id,
                    },
        ...
        var url = "{{ url('/') . '/' . auth()->user()->roles[0]->name . '/users' }}";
        $(document).on('click', '.add-new-user', function() {
            target = url + '/create';
            console.log(target);
            $("#addUserModal .modal-body").load(target, function() {
                $("#addUserModal").modal("show");
            });
        });
        $(document).on('click', '.edit-user', function() {
            user_id = $(this).data('id');
            target = url + '/' + user_id + '/edit';
            $("#editUserModal .modal-body").load(target, function() {
        ...
        $(document).on('click', '.show-user', function() {
            user_id = $(this).data('id');
            target = url + '/' + user_id;
            $("#showUserModal .modal-body").load(target, function() {
